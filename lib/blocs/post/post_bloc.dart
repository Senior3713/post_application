...
class PostBloc extends Bloc<PostEvent, PostState> {
  PostBloc() : super(PostInitial()) {
    ...
    on<IncrementViewCountEvent>(_incrementViewCount);
  }
  ...
  void _incrementViewCount(IncrementViewCountEvent event, Emitter emit) async {
    emit(PostLoading());

    await DBService.viewCountIncrement(event.count, event.postId, event.email, event.viewers);

    emit(const ViewCountIncrementSuccess());
  }
}
